...
const { unknownFile, databaseFile, synonimousFile, ignore02 } = require("./config")

const unknown = require(unknownFile)
...
class AnswerDatabase {
	 ...
	 */
	constructor() {
		const file = fs.readFileSync(databaseFile).toString()
...
class SynonimousProvider {
	...
	static load(){
		const file = fs.readFileSync(synonimousFile).toString()
		const lines = file.split("\n")
		this.scopes = []
		this.synonimous = []
